...
from pyframe.log.errors import Error
from pyframe.screen.colors import Color, Colors
from pyframe.screen.frames.border.border_type import Borders, BorderType, Direction
from pyframe.screen.frames.frame import Frame, get_box
from pyframe.screen.grid.cell import Cell
from pyframe.screen.grid.matrix import Grid
from pyframe.screen.grid.nested import max_len
from pyframe.screen.grid.point import Coord, Size
from pyframe.screen.window.window import keys_pressing
...
class Menu(Frame):
    def __init__(
        ...
        starting_selection: Optional[SwitchSelection] = None,
    ) -> None:
        super().__init__(cells, border_type)

        self.selections = selections

        self.selection = starting_selection or self.selections[0]
        ...
        self.selections.pop(self.selections.index(self.selection))
    ...
    def update(self):
        for switch_selection in self.selections:
            if switch_selection == self.selection:
                selection = (
                    switch_selection.selected
                )  # switch to a queue of off and the last one is selected
            else:
            ...
            if isinstance(selection, FrameSelection):
                ...
                # print("z", repr(self))
                # print("x", repr(self[selection.pos : selection.pos + selection.size]))
                # print("y", repr(Grid(Cell.from_str(selection.label))))

                self[selection.pos :] = Grid(Cell.from_str(selection.label))
    ...
    def move(self, direction: Direction, times: int = 1) -> None:
        for _ in range(times):
            for i, (selection, directions) in enumerate(
                self.selection.directions.items()
            ):
