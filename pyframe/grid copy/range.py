...
'2023-07-13 09:39 PM  July, 13, Thursday'
'Entries: 23/50 (Modified Often)'
...
# Coord(5, 6).y += 10
# print(rect_range(Coord(6, 9)))


# def array_rect_range(stop: Coord, start=None) -> list[list[Coord]]:
#     start = start or Coord(0, 0)
#     return [
#         [Coord(y, x) for x in range(start.x, stop.x + 1)]
#         for y in range(start.y, stop.y + 1)
#     ]


def selection_range(width: int, stop: Coord, start=None) -> list[Coord]:
    start = start or Coord(0, 0)
    res = [Coord(y, x) for y in range(start.y, stop.y + 1) for x in range(width)]
    res = res[start.x : -(width - (stop.x + 1))]
    return res


def center_range(outer_size: Coord, inner_size: Coord) -> slice:
