...
def level_out(
    rows: list[list[Cell]], alignment: Alignment = Alignment.LEFT
) -> list[list[Cell]]:
    """Level out the rows of the matrix making them all the same width."""
    max_length = max(len(row) for row in rows)
    ...
    for i, row in enumerate(rows):
        ...
        if alignment == Alignment.LEFT:
            row.extend(Cell.padding(max_length - row_length))
        elif alignment == Alignment.RIGHT:
            rows[i] = Cell.padding(max_length - row_length) + row
        elif alignment == Alignment.CENTER:
            left_padding = (max_length - row_length) // 2
            right_padding = max_length - row_length - left_padding
            rows[i] = Cell.padding(left_padding) + row + Cell.padding(right_padding)
    return rows
